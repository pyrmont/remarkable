...
(defn- check-unicode-punc
  "Check if captured UTF-8 bytes represent a Unicode punctuation character.
   Returns the bytes if punctuation, nil if not (for cmt to fail match)."
  [s]
  (if (punctuation/upunc? (bytes-to-codepoint s))
    s
    nil))
